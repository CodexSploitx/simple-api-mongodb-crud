...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await otpCol.insertOne({ userId: new ObjectId(auth.userId), code, eventKey: "reset_password_auth", used: false, attempts: 0, maxAttempts, createdAt: now, expiresAt });
    ...
    await outboxCol.insertOne({ eventKey: "reset_password", userId: new ObjectId(auth.userId), to: email, subject, html, queuedAt, status: "queued" });
    try {
